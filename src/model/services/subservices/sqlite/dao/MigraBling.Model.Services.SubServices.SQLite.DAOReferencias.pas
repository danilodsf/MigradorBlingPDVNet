...
const
  ...
  SQL_VARIACOES =
    ...
    'and M.MAT_REFERENCIA = :MAT_REFERENCIA ' + 'order by mat_referencia, mat_codigo';
  ...
  try
    Result := LerEntidade<TReferencia, TVariacao>(SQL, SQL_VARIACOES, SQL_SALDO_CONSOLIDA,
      ...
      begin
        ...
        AQueryVariacoes.ParamByName('PCODTABELA').AsInteger := LConfiguracao.TabelaPrecoPadrao;
        AQueryVariacoes.Open;
        LOrdem := 1;
        while not AQueryVariacoes.EOF do
        begin
          LReferencia.TemAoMenosUmaVariacaoValida := true;

          LVariacao := TVariacao.Create;
          LVariacao.ID := AQueryVariacoes.FieldByName('MAT_CODIGO').AsString;
          LVariacao.ID_Bling := AQueryVariacoes.FieldByName('ID_BLING').AsString;
          LVariacao.Referencia := AQueryVariacoes.FieldByName('MAT_REFERENCIA').AsString;

          LVariacao.CorStr := AQueryVariacoes.FieldByName('COR_DESCRICAO').AsString;
          LVariacao.Cor_Vinculo := AQueryVariacoes.FieldByName('COR_VINCULO_ID_BLING').AsString;
          LVariacao.Cor_ID_Bling := AQueryVariacoes.FieldByName('COR_ID_BLING').AsString;

          LVariacao.TamanhoStr := AQueryVariacoes.FieldByName('TAM_TAMANHO').AsString;
          LVariacao.Tamanho_Vinculo := AQueryVariacoes.FieldByName
            ('TAMANHO_VINCULO_ID_BLING').AsString;
          LVariacao.Tamanho_ID_Bling := AQueryVariacoes.FieldByName('TAM_ID_BLING').AsString;

          LVariacao.Preco := AQueryVariacoes.FieldByName('PRE_PRECO1').AsCurrency;
          ...
          begin
            ...
            LVariacao.Saldos.Add(LSaldo);
            AQuerySaldos.Next;
          end;

          LReferencia.Variacoes.Add(LVariacao);
          Inc(LOrdem);
          AQueryVariacoes.Next;
        end;
